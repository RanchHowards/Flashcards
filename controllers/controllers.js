...
module.exports.createUser = async (req, res) => { //validateRegistration is server side validation from JOI
...
module.exports.flashcard = (req, res) => { //don't need this middleware here, just trying it out	
    res.render('flashcard');
}
module.exports.showEditCards = async (req, res, next) => {
    const data = await User.findById(currentUser._id); //req.user._id
    if (!data) {
        throw new AppError('Cars not found', 404) //if we weren't throwing it to next, we'd return & pass it to next so doesn't execut the last line in the route
        ...
        User.updateOne({ _id: req.user._id },
            ...
            function (err, update) {
                if (err) {
                    ...
                    console.log("UNSET & SET: ", update);
                }
            })
    }
}

module.exports.postAddCards = (err, req, res, next) => { //i think this is an error hanlder, maybe can remove
    if (err) {
        console.log(req);
    ...
    csvtojson()
        ...
        .then(csvData => {
            let newObj = csvData.reduce((acc, next) => {
                let { front, back } = next;
                acc[front] = back;
                return acc;
            }, {})
            console.log(newObj);
            User.findByIdAndUpdate(currentUser._id, { 'words.new': newObj }, { new: true }, (err, update) => {
                if (err) {
                    console.log(err.message);
                    res.redirect('/addcards')
                }
                else res.redirect('/flashcard');
            })
        })
...
module.exports.putApi = async (req, res) => {
    ...
    User.updateOne({ _id: id }, { words: updatedDeck, firstRound: firstRound, session: session }, { new: true }, function (err, update) {
        if (err) {
            console.log(err);
        } else {
