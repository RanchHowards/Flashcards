...
$(document).ready(function () {
    ...
    $('#wrong').click(() => {
        if (deckStatus) {
            incorrectCount++;
            $('#incorrectProgress').css('width', incorrectCount / deckCount * 100 + "%");
            let front = $('#english').text();
            let back = $('#spanish').text();
            for (let i = 1; i < 11; i++) {
                if (decks[i][front]) {
    ...
    $('#newSession').click(() => {
        ...
        if (firstRound) {
            ...
            var subArr = [0, 2, 5, 9].map((val) => (session - val > 0) ? (session - val) : (10 + session - val));
        }
        combinedDecks = {
            ...decks.new,
            ...decks[subArr[0]],
...
async function fetch() { //must be adjusted for more users
