...
function updateBoth(button) {
  ...
  const oldName = $(button)
    .parent()
    .siblings('.front')
    .find('input')
    .attr('name')
  const newName = $(button).parent().siblings('.front').find('input').val()
  let newPath = oldName + newName //need to double check where these go
  let newValue = $(button).parent().siblings('.back').find('input').val()
